...
# Chaque plage dispose désormais de son orientation face à la mer (en degrés : 0=Nord, 90=Est, 180=Sud, 270=Ouest)
BEACHES = [
    {"name": "Anse des Soux", "lat": 46.6910, "lon": -2.3209, "facing": 210},      # Côte Sauvage (Sud-Ouest)
    {"name": "Plage des Vieilles", "lat": 46.6957, "lon": -2.3137, "facing": 200},  # Sud-Ouest
    {"name": "Grande Conche", "lat": 46.6946, "lon": -2.2850, "facing": 160},      # Sud-Est
    {"name": "Petite Conche", "lat": 46.7065, "lon": -2.2991, "facing": 180},      # Sud
    {"name": "Plage des Corbeaux", "lat": 46.6908, "lon": -2.2820, "facing": 90},   # Est
    {"name": "Marais Salés", "lat": 46.7127, "lon": -2.3103, "facing": 10},       # Nord
    {"name": "Ker Châlon", "lat": 46.7196, "lon": -2.3351, "facing": 0},        # Nord
    {"name": "Plage des Sapins", "lat": 46.7174, "lon": -2.3159, "facing": 20},     # Nord-Nord-Est
    {"name": "Anse des Fontaines", "lat": 46.6895, "lon": -2.3334, "facing": 220},  # Sud-Ouest
    {"name": "Plage de la Gournaise", "lat": 46.7337, "lon": -2.3809, "facing": 310},# Nord-Ouest
    {"name": "Plage du But", "lat": 46.7257, "lon": -2.3969, "facing": 340},      # Nord-Ouest
    {"name": "Plage des Sabias", "lat": 46.7034, "lon": -2.3739, "facing": 30}     # Nord-Est (Encaissée / Protégée)
]
...
with col1:
    ...
    for b in BEACHES:
        # Calcul de l'écart angulaire exact entre la provenance du vent et l'orientation de la plage
        diff = abs((wd - b["facing"] + 180) % 360 - 180)
        
        # Particularité pour les Sabias : crique encaissée offrant une forte tolérance aux vents de secteur Sud/Ouest/Nord
        if b["name"] == "Plage des Sabias":
            # Si le vent vient de terre ou de travers de manière générale, elle est ultra-protégée
            if diff > 60 or (180 <= wd <= 310): 
                is_good = True
            else:
                is_good = False
        else:
            # Règle générale vectorielle : > 100° signifie que le vent vient de la terre (abritée)
            is_good = diff > 105
...
if st.button("📍 Obtenir ma position GPS (Smartphone)"):
    components.html("""<script>navigator.geolocation.getCurrentPosition(p => {window.location.search="?lat="+p.coords.latitude+"&lon="+p.coords.longitude})</script>""", height=0)    {"lat": 46.700, "lon": -2.319}, {"lat": 46.718, "lon": -2.360},
    {"lat": 46.705, "lon": -2.350}, {"lat": 46.710, "lon": -2.300}
]
...
with col1:
    ...
    for b in BEACHES:
        if card in b["good"]: icon_html = svg_up
        elif card in b["bad"]: icon_html = svg_down
        else: icon_html = svg_right
